...
static void ice_melting_bfb(){
  ...
  Kokkos::parallel_for(RangePolicy(0, 1), KOKKOS_LAMBDA(const Int& i) {
    // Init pack inputs
    Spack rho,t,pres,rhofaci,f1pr05,f1pr14,xxlv,xlf,dv,sc,mu,kap,qv,qitot_incld,nitot_incld
    ...
    // Validate results
    for (Int s = 0; s < Spack::n; ++s) {
       REQUIRE(IceMelt[s].qimlt == qimlt_host(s).qimlt);
       REQUIRE(IceMelt[s].nimlt == qimlt_host(s).nimlt);
     }
}
